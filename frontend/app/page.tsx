...
type MenuSection = { id: string; title: string; items: MenuItemData[] };
type MenuFromApi = { restaurant: string; sections: MenuSection[] };

/** Fallback static menu (used until API loads or if API fails) */
const MENU_FALLBACK: MenuFromApi = {
...
export default function Home() {
  ...
  const [menu, setMenu] = useState<MenuFromApi | null>(null);

  useEffect(() => {
    fetch(`${API_BASE}/menu/sections`)
      .then((res) => {
        if (!res.ok) throw new Error("Failed to load menu");
        return res.json();
      })
      .then((data: MenuFromApi) => {
        if (data?.sections?.length) setMenu(data);
        else setMenu(MENU_FALLBACK);
      })
      .catch(() => setMenu(MENU_FALLBACK));
  }, []);

  const menuData = menu ?? MENU_FALLBACK;
  const sectionsToShow =
    selectedCategory === ALL_ID
      ? menuData.sections
      : menuData.sections.filter((s) => s.id === selectedCategory);
          ...
          <header className="flex items-center justify-between px-5 py-3">
            <div className="flex items-center gap-2">
              ...
              <div className="flex flex-col">
                <p className="text-sm font-semibold text-[var(--foreground)]">{menuData.restaurant}</p>
